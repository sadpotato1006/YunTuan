...
function normalizeMessagePolicy(value) {
  ...
  const remainingBeforeReply = limited
    ? Math.max(0, Math.min(3, Number(source.remainingBeforeReply) || 0))
    : null;
...
function mergeMessages(first, second) {
  const byId = new Map();
  (Array.isArray(first) ? first : []).concat(Array.isArray(second) ? second : [])
    .forEach(message => {
