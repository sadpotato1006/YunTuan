...
“云团”是面向随迁老人的陪伴类产品。小程序主界面只保留“设备”和“伙伴”两个页面：设备页负责挂件状态、个人名片和低频隐私工具，伙伴页集中 AI 陪伴、最近相遇、朋友消息与招呼。本仓库同时提供可部署的聊天云函数、Mock 数据，以及云团 BLE 控制、录音上行和语音播放实现。真实 AI、ASR 和 TTS 仍需云端环境变量，真实 BLE 功能需要与符合协议的挂件固件进行真机联调。
...
也可以在各环境配置的 `serviceBackendModes` 中为不同业务单独指定模式。当前三个环境默认由聊天调用云函数，设备使用本机 BLE：
...
```js
backendMode: "mock",
serviceBackendModes: { chat: "mock", device: "ble" }
```

Mock 模式不会初始化或调用 `wx.cloud`，因此无需创建云环境。面向老人的设备页只展示连接、电量和常用控制；设备型号、固件、硬件和协议版本统一放进页面底部的“开发者工具”。该入口还集中提供连接管理、模拟挂件、BLE 联调和故障诊断，可以在没有真实硬件时验证 BLE 1.7 控制协议。模拟挂件只声明其实际提供的控制、相遇和提醒能力，不伪装音频服务。
...
4. 在开发者工具的 `cloudfunctions` 目录中，依次右键 `chat`、`device`、`social`、`social-cleanup`。
5. 选择“上传并部署：云端安装依赖”。
6. 重新编译小程序。
...
数据量增长前建议在控制台的“索引管理”中按下表创建索引。字段方向严格按表中标注选择；未单独标注的单字段索引选“升序”。索引属性均选“非唯一”，系统自动创建的 `_id_` 和 `_openid_1` 保持不动。仓库中的 `database/required-indexes.json` 固定记录当前查询必须使用的关键组合索引，部署或切换环境时应一并核对。
...
| `social_tokens` | `idx_tokens_expires` | 单字段索引 | `expiresAt` |
| `social_encounter_refs` | `idx_refs_expires` | 单字段索引 | `expiresAt` |
| `social_resolve_usage` | `idx_resolve_usage_updated` | 单字段索引 | `updatedAt` |
...
`social-cleanup` 是每天凌晨 3 点运行的定时云函数，用于物理删除过期匿名 Token、相遇互动凭证、临时二维码和旧频控记录。已经解除的伙伴关系保留 90 天后，会连同对应会话和消息一起删除；仍然有效的伙伴聊天不会被定时清理。部署后请在云开发控制台确认 `daily-social-lifecycle-cleanup` 触发器已经创建成功。删除全部个人数据时，云函数会分页读取全部联系方式和文件登记，不再只处理前 100 条。
...
挂件连接后，小程序读取本次启动的匿名 Token 并登记到云端，之后会在恢复设备页和到期前自动续期。两台都开启社交模式的挂件靠近并满足 RSSI 规则后，双方挂件按个人提醒设置反馈，并向已连接的小程序发送对方 Token；小程序再用 Token 查询对方的公开名片并写入最近 30 次相遇记录。相遇记录按匿名稳定伙伴键汇总，同一个人只显示一次并累计相遇次数；对于已经互相确认的伙伴，云端不会再次签发招呼凭证，小程序也不会再次提示发送招呼。尚未认识的用户可以通过匿名互动凭证打招呼；收到的招呼直接显示在“伙伴”页朋友列表下方，不再进入独立页面，两个列表可以分别折叠，朋友始终优先显示。对方明确点击“认识 TA”后，云端才会建立双方专属的一对一会话。伙伴聊天当前只支持用户主动确认发送的文字、表情和快捷开场，不包含录音、语音消息、语音转文字或音视频通话。客户端不会获得对方 OpenID、设备 ID、精确位置或真实身份。
...
设备页底部提供默认折叠的“隐私与数据”入口，集中放置隐私说明、关于云团、清除本机缓存和删除全部个人数据。删除个人数据会移除社交名片与头像、联系方式二维码、招呼、伙伴关系、伙伴会话和消息、屏蔽与举报记录，以及 AI 聊天幂等/用量记录，并在成功后清除本机缓存；该操作可重复调用，以便网络中断后再次完成删除。

“开发者工具”页提供脱敏真机诊断导出：本机最多保存最近 160 条 BLE 关键事件、语音阶段切换、云函数耗时和首包播放耗时。报告不会包含聊天正文、录音、联系方式、OpenID、设备序列号或社交 Token，可直接复制给开发人员定位卡顿阶段。

## 模块边界

较大的业务文件已经按职责拆出独立模块：社交云函数的校验与脱敏在 `social-utils.js`，收件箱分页在 `social-inbox.js`，定时回收在 `social-cleanup`；聊天云函数的音频编解码在 `audio-codec.js`；小程序端的 BLE 展示辅助、挂件音频协议辅助、设备视图、伙伴聊天格式化和语音耗时计算也分别位于独立文件。页面和主服务保留流程编排，协议解析、数据整形和生命周期处理不再继续堆入入口文件。
...
1. 先保存自己的社交名片，再从设备页底部进入“开发者工具”，点击“加载模拟挂件”。
2. 返回设备页开启社交模式，再进入“开发者工具”点击“创建测试伙伴并模拟相遇”。云函数会为当前账号创建一位隔离的“云团测试伙伴”，并通过模拟挂件注入标准相遇事件。
3. 前往伙伴页，从“最近相遇”入口查看名片并点击“打个招呼”；测试伙伴会自动接受，随后可进入会话。
...
2. A、B 各自保存社交名片，然后从设备页进入“开发者工具”，点击“加载模拟挂件”。
